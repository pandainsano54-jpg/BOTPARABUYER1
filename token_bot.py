...
- ✅ Comando /token_status para ver estado de tokens
...
def get_token_status(token: str) -> Tuple[bool, str]:
    """Check if token is expired. Returns (is_expired, expires_at_formatted)"""
    try:
    ...
    try:
        ...
        if is_on_cooldown:
            ...
            await interaction.response.send_message(
                ...
                f">>> 🔒 *20-minute cooldown so bot doesnt broke*",
                ephemeral=True,
            )
...
@tree.command(
    name="token_status",
    description="Check status of all tokens"
)
async def token_status_cmd(interaction: discord.Interaction):
    """
    Command /token_status - Check all tokens status
    ...
    try:
        ...
        message = "╔════════════════════════════════════════╗\n"
        message += "║        TOKEN POOL STATUS               ║\n"
        message += "╚════════════════════════════════════════╝\n\n"
        
        for idx, token_data in enumerate(tokens_data["tokens"], 1):
            token_id = token_data.get("id", f"token_{idx}")
            token = token_data.get("token", "")
            
            is_expired, expires_at = get_token_status(token)
            
            status_icon = "❌" if is_expired else "✅"
            status_text = "EXPIRED" if is_expired else "VALID"
            
            message += f"{status_icon} **Token {idx}: {token_id}**\n"
            message += f"   Status: `{status_text}`\n"
            message += f"   Expires: `{expires_at}`\n\n"
        
        message += "───────────────────────────────────────\n"
        message += f" **Total Tokens:** `{len(tokens_data['tokens'])}`\n"
        message += f" **Last Check:** `{time.strftime('%H:%M:%S')}`\n"
        message += "───────────────────────────────────────\n"
        
        await interaction.response.send_message(message, ephemeral=True)
        print(f"[BOT] ✅ Token status checked by {interaction.user}")
        ...
        print(f"[BOT] ❌ Error in /token_status command: {e}")
        traceback.print_exc()
...
async def on_ready():
    ...
    try:
        ...
        print(f"   • /token_status (Check token status - Admin only)")
